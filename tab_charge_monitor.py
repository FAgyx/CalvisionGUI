...
scale_factor = 1.0/4096

class ChargeMonitorWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        # print(f"[ChargeMonitor] convert_dat_to_root took {t1 - t0:.3f} s")

        if self.n_new_sampled_events <= 0:
            print("[ChargeMonitor] No new events")
        ...
        if not fin or fin.IsZombie() or not fin.IsOpen():
            print("[ChargeMonitor] Failed to open ROOT file")
            return

        tree = fin.Get("pulse")
        if tree is None:
            print("[ChargeMonitor] 'pulse' tree not found")
            fin.Close()
            return

        # t2 = time.perf_counter()
        # print(f"[ChargeMonitor] Opening ROOT file took {t2 - t1:.3f} s")
        ...
        for ch in range(N_Sipm_Channels):
            ...
            # print(f"[ChargeMonitor] Processing channel {ch} (first_entry={first_entry}, n_entries={n_new_sampled_events})") # sampled events
            
            # Update last processed event index for this channel
            new_event_indices[ch] = end_point

            branch_name = f"charge_ch{ch}"
            
            charge_for_ch = []
            if tree.GetBranch(branch_name):
                for i in range(first_entry, end_point):
                    tree.GetEntry(i)
                    charge_for_event = getattr(tree, branch_name)
                    charge_for_ch.extend(list(charge_for_event))
                
                result[ch] = charge_for_ch

        # t3 = time.perf_counter()
        # print(f"[ChargeMonitor] Loop over channels took {t3 - t2:.3f} s")
        ...
        print(f"[ChargeMonitor] Total ChargeMonitorWorker.run() time {t4 - t0:.3f} s")
    
    def stop(self):
        self._running = False

class tab_charge_monitor(QtCore.QObject):
    def __init__(self, run_config, status, MainWindow):
        ...
        self.bin_edges = np.linspace(0, 10, 101)  # 100 bins 
        ...
        self.ref_hist_data = load_reference_histogram(ref_file, "charge", self.bin_edges)
    ...
    def start_worker(self, root_filename, n_new_sampled_events):
        ...
        if self.is_running:
            ...
            self.worker = ChargeMonitorWorker(
                dat_file, 
    ...
    def on_histograms_ready(self, payload):
        ...
        for ch in range(N_Sipm_Channels):
            if ch in result:
                ...
                if len(data) != 0:
                    print(f"[tab_charge_monitor] Channel {ch} updated with {len(data)} new charge values")
    ...
    def start_new_run(self):
        ...
        # print("[tab_charge_monitor] New run initialized")
