...
class ClusterMonitorWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        # print(f"[ClusterMonitor] convert_dat_to_root took {t1 - t0:.3f} s")

        if self.n_new_sampled_events <= 0:
            print("[ClusterMonitor] No new events")
        ...
        if not fin or fin.IsZombie() or not fin.IsOpen():
            print("[ClusterMonitor] Failed to open ROOT file")
            return

        tree = fin.Get("pulse")
        if tree is None:
            print("[ClusterMonitor] 'pulse' tree not found")
            fin.Close()
            return

        # t2 = time.perf_counter()
        # print(f"[ClusterMonitor] Opening ROOT file took {t2 - t1:.3f} s")
        ...
        for ch in range(N_Sipm_Channels):
            ...
            # print(f"[ClusterMonitor] Processing channel {ch} (first_entry={first_entry}, n_entries={n_new_sampled_events})") # sampled events
            
            # Update last processed event index for this channel
            new_event_indices[ch] = end_point

            branch_name = f"cluster_count_ch{ch}"
            
            cluster_for_ch = []
            if tree.GetBranch(branch_name):
                for i in range(first_entry, end_point):
                    tree.GetEntry(i)
                    cluster_for_event = getattr(tree, branch_name)
                    if cluster_for_event > 0:
                        cluster_for_ch.append(cluster_for_event)
                
                result[ch] = cluster_for_ch

        # t3 = time.perf_counter()
        # print(f"[ClusterMonitor] Loop over channels took {t3 - t2:.3f} s")
        ...
        print(f"[ClusterMonitor] Total ClusterMonitorWorker.run() time {t4 - t0:.3f} s")
    
    def stop(self):
        self._running = False

class tab_cluster_monitor(QtCore.QObject):
    def __init__(self, run_config, status, MainWindow):
        ...
        self.ref_hist_data = load_reference_histogram(ref_file, "cluster_count", self.bin_edges)
    ...
    def start_worker(self, root_filename, n_new_sampled_events):
        ...
        if self.is_running:
            ...
            self.worker = ClusterMonitorWorker(
                dat_file, 
    ...
    def on_histograms_ready(self, payload):
        ...
        for ch in range(N_Sipm_Channels):
            if ch in result:
                ...
                if len(data) != 0:
                    print(f"[on_histograms_ready] Channel {ch} updated with {len(data)} new clusters")
    ...
    def setup_UI(self, MainWindow):
        ...
        for i in range(N_Sipm_Channels):
            ...
            plot.setLabel('bottom', 'num_clusters')
    ...
    def start_new_run(self):
        ...
        # print("[tab_cluster_monitor] New run initialized")
